...
.stButton > button {
    ...
    box-shadow: 0 2px 8px rgba(192, 123, 74, 0.25) !important;
}
...
.btn-got-it button {
    ...
    box-shadow: 0 2px 8px rgba(74, 124, 95, 0.25) !important;
}
.btn-got-it button:hover { background: #3a6349 !important; }
.btn-need-review button {
...
.card-question {
    ...
    color: var(--text-primary); line-height: 1.55; margin-bottom: 0.9rem;
}
...
.study-question {
    font-family: 'Lora', serif; font-size: 1.4rem; font-weight: 500;
    color: var(--text-primary); line-height: 1.45; max-width: 560px; margin: 0 auto;
}
.study-answer {
...
.score-number {
    font-family: 'Lora', serif; font-size: 4rem; font-weight: 600;
...
with tab_gen:
    ...
    with input_tab_pdf:
        ...
        with col1:
            num_cards_pdf = st.slider("How many cards?", 5, 20, 10, key="slider_pdf")
        with col2:
            difficulty_pdf = st.selectbox("Difficulty", ["Easy", "Medium", "Hard"], index=1, key="diff_pdf")
        with col3:
                ...
                try:
                    ...
                    for i in range(1, len(cards) + 1):
                        progress_placeholder.markdown(
                            f'<div class="gen-counter">{i} of {len(cards)} cards ready</div>',
                            unsafe_allow_html=True)
                        time.sleep(0.07)
    ...
    with input_tab_text:
        ...
        col1, col2, col3 = st.columns([2, 1, 2])
        with col1:
            num_cards_text = st.slider("How many cards?", 5, 20, 10, key="slider_text")
        with col2:
            difficulty_text = st.selectbox("Difficulty", ["Easy", "Medium", "Hard"], index=1, key="diff_text")
        with col3:
            topic_text = st.text_input("Topic (optional)", placeholder="e.g. Photosynthesis", key="topic_text")
        st.markdown('</div>', unsafe_allow_html=True)
                ...
                try:
                    with st.spinner("Building your flashcards..."):
                        cards = run_full_pipeline(
                            ...
                            num_cards=num_cards_text,
                            difficulty_hint=get_difficulty_hint(difficulty_text)
                        )
                    st.session_state.flashcards = cards
                    st.session_state.quiz_questions = []
                    st.session_state.study_started = False
    ...
    with input_tab_yt:
        st.markdown('<div class="input-card">', unsafe_allow_html=True)
        yt_url = st.text_input("YouTube link", placeholder="https://www.youtube.com/watch?v=...", label_visibility="visible")
        translate_hindi = st.checkbox("Translate Hindi → English", value=False)
        st.markdown("<br>", unsafe_allow_html=True)
        col1, col2, col3 = st.columns([2, 1, 2])
        with col1:
            num_cards_yt = st.slider("How many cards?", 5, 20, 10, key="slider_yt")
        with col2:
            difficulty_yt = st.selectbox("Difficulty", ["Easy", "Medium", "Hard"], index=1, key="diff_yt")
        with col3:
            topic_yt = st.text_input("Topic (optional)", placeholder="e.g. Recursion", key="topic_yt")
        st.markdown('</div>', unsafe_allow_html=True)
                    ...
                    try:
                        with st.spinner("Building your flashcards..."):
                            cards = run_full_pipeline(
                                ...
                                num_cards=num_cards_yt,
                                difficulty_hint=get_difficulty_hint(difficulty_yt)
                            )
                        st.session_state.flashcards = cards
                        st.session_state.quiz_questions = []
                        st.session_state.study_started = False
...
with tab_study:
    ...
    if not cards:
        ...
        <div style="padding: 1.5rem 0;">
            <div style="font-family: Lora, serif; font-size: 1.2rem; font-weight: 500; color: #2C1F0E; margin-bottom:0.5rem;">
                {len(due_cards)} of {len(cards)} cards due for review
            </div>
            <div style="color: #7A6248; font-size: 0.9rem; margin-bottom: 1.5rem;">
                {mastered} cards mastered across all your sessions
            </div>
        </div>
        """, unsafe_allow_html=True)
